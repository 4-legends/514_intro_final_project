...
if __name__ == '__main__':
	...
	Robot = robot('kinova')
	Robot.planner_type("RRT")
	Robot.get_Object([0.02, 1.5, 2], [-0.61, 0, 0, 1.0], "wall") # wall
	Robot.get_Object([2, 2, 0.02], [0, 0, 0, 1.0], "floor") # wall
	Robot.get_Object([0.5, 0.6, 0.1], [-0.5, 0, 0.7, 1.0], "camera bar") # wall
	rospy.sleep(3)
	...
	while not rospy.is_shutdown():
		...
		priority = float(priority.replace(']"', ''))


		# Go here and grab puzzle
		
		
		if priority == 1 and len(grab_point) == 0:
